...
public class WorkshopImpl implements Workshop {
  ...
  private final Map<WorkplaceId, WorkplaceNode> workplaces;  // Immutable.
  private final Map<Long, Worker> workers = new HashMap<>();

  public WorkshopImpl(Collection<Workplace> workplaces) {
    this.workplaces = workplaces.stream().collect(Collectors.toUnmodifiableMap(Workplace::getId, x -> new WorkplaceNode(this, x)));
  }
  ...
  public Workplace enter(WorkplaceId newWorkplaceId) {
    lock.lock();
    var worker = workers.computeIfAbsent(getWorkerId(), Worker::new);
  ...
  public Workplace switchTo(WorkplaceId newWorkplaceId) {
    ...
    WorkplaceNode newWorkplace = workplaces.get(newWorkplaceId);
